...
import { getStatLabel } from '@/game/core/statUtils';
...
interface ChoiceResultModalProps {
  ...
  followUp?: string;
  isWaitingBackend?: boolean;
}

export function ChoiceResultModal(props: ChoiceResultModalProps) {
  const { isOpen, onClose, choiceText, statChanges, followUp, isWaitingBackend = false } = props;
  ...
  const formatStatChange = (key: string, value: number) => {
    ...
    if (key === 'money' || key === 'totalMoneyEarned') {
      return (
