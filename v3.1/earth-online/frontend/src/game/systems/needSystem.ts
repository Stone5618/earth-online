 ...
 * 获取心情信息
 */
export function getMoodInfo(mood: number): { color: string; label: string } {
  if (mood >= 80) return { color: '#00FF88', label: '愉悦' };
  if (mood >= 60) return { color: '#7CFC00', label: '良好' };
  if (mood >= 40) return { color: '#FFD700', label: '一般' };
  if (mood >= 20) return { color: '#FF8C00', label: '低落' };
  return { color: '#FF4B4B', label: '沮丧' };
}
