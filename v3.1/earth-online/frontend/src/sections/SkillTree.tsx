...
const classColorMap: Record<string, { bg: string; border: string; icon: string }> = {
  ...
  gold: { bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', icon: 'text-yellow-400' },
};

const defaultClassColors = { bg: 'bg-gray-500/10', border: 'border-gray-500/30', icon: 'text-gray-400' };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {classes.map((cls, index) => {
              const cc = classColorMap[cls.color] || defaultClassColors;
              return (
              <GlassCard key={cls.name} delay={0.2 + index * 0.1} className="group">
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-xl ${cc.bg} border ${cc.border}`}>
                    <cls.icon className={`w-6 h-6 ${cc.icon}`} />
                  </div>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {easterEggs.map((egg, index) => (
              ...
              >
                ...
                <span className="text-white/60 text-xs">{egg.desc}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
